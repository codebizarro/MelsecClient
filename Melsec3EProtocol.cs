...
namespace System.Net.Melsec
{
    public sealed class Melsec3EProtocol : MelsecEthProtocol
    {
        ...
        public override float ReadReal(ushort point, MelsecDeviceType DeviceType)
        {
            ...
            byte[] sendbuffer = new byte[] {0x50,0x00,NetNo,PcNo,(byte)destinationCpu,0x03,0x00,0x0C,0x00,0x10,0x00,
				0x01,0x04,0x00,0x00,
        ...
        public override float[] ReadReal(ushort[] point, MelsecDeviceType DeviceType)
        {
            ...
            byte[] buff1 = new byte[] {0x50,0x00,NetNo,PcNo,(byte)destinationCpu,0x03,0x00,len[0],len[1],0x10,0x00,
        ...
        public override void WriteReal(ushort point, float val, MelsecDeviceType DeviceType)
        {
            ...
            byte[] sendbuffer = new byte[] {0x50,0x00,NetNo,PcNo,(byte)destinationCpu,0x03,0x00,0x10,0x00,0x10,0x00,
				0x01,0x14,0x00,0x00,
				addr[0],addr[1],addr[2],
				(byte)DeviceType,
				0x02,0x00,
        ...
        public override void WriteWord(ushort point, ushort val, MelsecDeviceType DeviceType)
        {
            ...
            byte[] sendbuffer = new byte[] {0x50,0x00,NetNo,PcNo,(byte)destinationCpu,0x03,0x00,0x0E,0x00,0x10,0x00,
				0x01,0x14,0x00,0x00,
				addr[0],addr[1],addr[2],
				(byte)DeviceType,
				0x01,0x00,
        ...
        public override void WriteByte(ushort point, bool state, MelsecDeviceType DeviceType)
        {
            ...
            byte[] sendbuffer = new byte[] {0x50,0x00,NetNo,PcNo,(byte)destinationCpu,0x03,0x00,0x0D,0x00,0x10,0x00,
				0x01,0x14,0x01,0x00,
				addr[0],addr[1],addr[2],
				(byte)DeviceType,
				0x01,0x00,
        ...
        public override void WriteByte(ushort[] point, bool[] state, MelsecDeviceType DeviceType)
        {
            ...
            if (state.Length == 0)
                throw new Exception(Globals.NO_DATA_WRITE);
            ushort count = (ushort)point.Length;
            byte[] sendbuffer = new byte[16 + count * 5];
            byte[] len = GetRequestDataLength(sendbuffer.Length - ERROR_CODE_POSITION);
            byte[] buff1 = new byte[] {0x50,0x00,NetNo,PcNo,(byte)destinationCpu,0x03,0x00,len[0],len[1],0x10,0x00,
        ...
        {
            byte[] sendbuffer = new byte[] {0x50,0x00,NetNo,PcNo,(byte)destinationCpu,0x03,0x00,0x06,0x00,0x10,0x00,
				0x17,0x16,
				0x00,0x00};
            SendBuffer(sendbuffer);
        }
        ...
        public override void Run(bool forced, ClearMode mode)
        {
            ...
            byte[] sendbuffer = new byte[] {0x50,0x00,NetNo,PcNo,(byte)destinationCpu,0x03,0x00,0x0A,0x00,0x10,0x00,
				0x01,0x10,
				0x00,0x00,
                frcd,0x00,
                (byte)mode, 0x00};
        ...
        {
            byte frcd = (forced) ? frcd = 0x03 : frcd = 0x01;
            byte[] sendbuffer = new byte[] {0x50,0x00,NetNo,PcNo,(byte)destinationCpu,0x03,0x00,0x08,0x00,0x10,0x00,
				0x03,0x10,
				0x00,0x00,
                frcd,0x00};
            SendBuffer(sendbuffer);
